...
INSERT INTO Presentations(title, presenterID, levelID, eventID, trackID) VALUES ('Coffee Break', (SELECT presenterID FROM Presenters WHERE (attendantID = (SELECT attendantID FROM Attendants WHERE firstName = (SUBSTRING('Robert Pearl', 1, CHARINDEX(' ', 'Robert Pearl') - 1)) AND lastName = (REVERSE(SUBSTRING(REVERSE('Robert Pearl'), 1, CHARINDEX(' ', REVERSE('Robert Pearl')) - 1)))))),  (SELECT levelID  FROM Levels WHERE levelDifficulty LIKE 'Non-Technical'), (SELECT eventID FROM Events WHERE eventName LIKE 'Budapest'), 2)
